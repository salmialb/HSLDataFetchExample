...
def getData():
    ...
    headers = {'Digitraffic-User': 'Trainguy/PythonTest'}           
    ...
    for date in marchDates:                                                                             #Loop through March 
        ...
        if len(r.json()) >= 1: responses.append(r.json()[0])                                            #No trains on Sundays. Check that there actually is something in the response before appending.                                                                       
...
def saveToCSV(df):
    ...
    file_dir = os.path.dirname(os.path.abspath(__file__))          #Get path of the working directory
    file_path = os.path.join(file_dir, file_dir, 'trainData.csv')  #Join the path of the working directory with the desired file name
    df.to_csv(file_path, index=False,encoding='ISO-8859-1')        #Save file
